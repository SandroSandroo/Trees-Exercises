...
class BinaryTree {
  ...
  minDepth() {
    ...
    function minDepthHelper(node) {
      ...
      return (
        Math.min(minDepthHelper(node.right), minDepthHelper(node.left)) + 1
      );
    }
  ...
  maxDepth() {
    ...
    function maxDepthHelper(node) {
      ...
      return (
        Math.max(maxDepthHelper(node.right), maxDepthHelper(node.left)) + 1
      );
    }
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let shouldReassignClosest = currentNode < closest || closest === null;
